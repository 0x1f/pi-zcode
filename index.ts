...
import { REGIONS, codingUrl, errorText, makeProvider, quotaReport, timeText, type Region } from "./core.ts";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("zcode-safe", {
    description: "浏览器登录的 Coding Plan：cn|intl status|refresh|quota；cancel 取消诊断。",
    handler: async (args, ctx) => {
      ...
      if ((region !== "cn" && region !== "intl") || !["status", "refresh", "quota"].includes(action) || extra) {
        ctx.ui.notify("用法：/zcode-safe cn|intl status|refresh|quota；/zcode-safe cancel。登录请用 /login zcode-cn 或 /login zcode-intl，在浏览器授权。", "warning");
        return;
      }
